    ...
    public class PlaceSpace
    {
        ...
        public static List<List<int>> PlaceProgramsInSite(List<ProgramData> progData, List<Cell> cells, List<Point2d> PolyPoints, double factor = 5, int num = 5)
        {
            ...
            for (int i = 0; i < selectedIndices.Count; i++)
            {
                ...
                // calc square edge
                double side = Math.Sqrt(progData[i].UnitArea * factor * progData[i].Quantity);
                sqEdge.Add(side);
            ...
            for (int i = 0; i < selectedIndices.Count; i++)
            {
                ...
                while (!progData[i].IsAreaSatisfied && AnyCellAvailable(cells))
                {

                    List<int> cellIndices = cellProgramsList[i];
                    for (int j = 0; j < cells.Count; j++)
                    {
                        if (cells[j].CellAvailable)
                        {
