...
namespace SpacePlanning
{
    public class SplitObject
    {
        ...
        public static Dictionary<string, object> DeptSplitRefined(Polygon2d poly, List<DeptData> deptData, int recompute = 1)
        {
            ...
            for (int i = 0; i < sortedD.Count; i++)
            {
                ...
                while (areaLeftOverToAdd > limit && leftOverPoly.Count > 0)
                {
                    ...
                    {
                        Dictionary<string, object> splitReturn = RecursiveSplitByArea(currentPolyObj, areaLeftOverToAdd,dir);
            ...
            do
            {
                ...
                polyAfterSplitting = RecursiveSplitByArea(currentPolyObj, area);
        ...
        //RECURSIVE SPLITS A POLY
        [MultiReturn(new[] { "PolyAfterSplit", "AreaEachPoly", "EachPolyPoint" })]
        public static Dictionary<string, object> RecursiveSplitByAreaOld(Polygon2d poly, double area,int dir, int recompute = 1)
        {
            ...
            */
            ...
            while (Math.Abs(areaDifference) > limit)
            {
                polyAfterSplitting = EdgeSplitWrapper(currentPoly, distance, dir);
                double area1 = GraphicsUtility.AreaPolygon2d(polyAfterSplitting[0].Points);
                ...
                {
                    areaDifference = area - area2;
                    distance = DistanceEditBasedOnRatio(distance, areaPoly, area2, area, setSpan, areaDifference);
                    Trace.WriteLine("Updated Distance for 2 is : " + distance);
                    Trace.WriteLine("Area Difference found for 2 is : " + areaDifference);
                }
