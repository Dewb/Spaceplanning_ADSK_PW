...
namespace SpacePlanning
{
    public class Circulation
    {
        ...
        public static Dictionary<string, object> CirculationMakeInPrograms(List<Polygon2d> polyProgList, List<Line2d> lineList,  double distance = 8, bool tag = true, double ratio = 3)
        {
            ...
            for (int i = 0; i < lineList.Count; i++)
            {
                ...
                for (int j = 0; j < polyProgList.Count; j++)
                {
                    ...
                    if (!tag)
                    {
                        Dictionary<string, object> splitResult = BuildLayout.SplitByLineMake(progPoly, splitter, distance);
        ...
        public static Dictionary<string, object> CirculationMake(List<DeptData> deptData, List<List<Line2d>> lineList, double distance = 8, bool tag = true)
        {
            ...
            for (int i = 0; i < cleanLineList.Count; i++)
            {
                ...
                for (int j = 0; j < allDeptPolyList.Count; j++)
                {
                    ...
                    if (!tag)
                    {
                        Dictionary<string, object> splitResult = BuildLayout.SplitByLineMake(deptPoly, splitter, distance);
                        List<Polygon2d> polyAfterSplit = (List<Polygon2d>)(splitResult["PolyAfterSplit"]);
                        if (polyAfterSplit != null)
                        {
                            double areaA = GraphicsUtility.AreaPolygon2d(polyAfterSplit[0].Points);
