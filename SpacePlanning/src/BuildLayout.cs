    ...
    public static class BuildLayout
    {
        ...
        public static Dictionary<string, object> PlacePrograms(List<DeptData> deptData, double primaryProgramWidth = 30, int recompute = 0)
        {
            ...
            for(int i = 0; i < deptData.Count; i++)
            {
                if (i == 0)
                {                    
                    Dictionary<string, object> placedPrimaryProg = PlaceInpatientPrograms2(deptData[i].PolyAssignedToDept, deptData[i].ProgramsInDept, primaryProgramWidth, recompute);
