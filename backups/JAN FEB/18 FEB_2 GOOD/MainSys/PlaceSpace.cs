...
namespace SpacePlanning
{
    public class PlaceSpace
    {
        ...
        //public functions
        [MultiReturn(new[] { "DepartmentDataProgramsAdded" })]
        public static Dictionary<string, object> DeptsAndProgsOnSite(List<DeptData> deptData, List<Cell> cells, List<Point2d> PolyPoints, int tag = 0)
        {


            //1 get dept and make new dept lists DONE

            //2 place depts in site ( returns updated dept data only with cell info and cell added )

            //3 place programs in dept ( returns updated dept data with program lists embedded which could satisfy the cells the dept had

            //4 return the updated dept
            ...
            //newDeptDataList = AssignProgramstoDept
            ...
            for(int i = 0; i < newDeptDataList.Count; i++)
            {
                List<ProgramData> progData = newDeptDataList[i].ProgramsInDept;
                Dictionary<string,object> placedItems = PlaceProgramsInSite(progData, cells, PolyPoints, progData.Count, 0);
                //newDeptDataList[i].ProgramsInDept = (List<ProgramData>)placedItems["ProgramData"];
