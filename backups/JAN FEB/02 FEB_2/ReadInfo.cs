...
namespace SpacePlanning
{
    public class ReadInfo
    {
        ...
        public static Dictionary<string, object> pushBubbles(List<Point> ptList, List<List<string>> dataStack, List<double> radiusList,int iteration)
        {
            List<Circle> cirList = new List<Circle>();
            for (int i=0; i < iteration; i++)
            {
                for(int j =0; j< ptList.Count; j++)
                {
                    for (int k = 0; k < ptList.Count; k++)
                    {
                        ...
                        Vector vec1 = Vector.ByTwoPoints(ptList[j], ptList[k]);
                        vec1 = vec1.Normalized();
                        vec1 = vec1.Scale(diff1);
                        Point p1 = ptList[j].Add(vec1);
                        ptList[j] = p1;

                        Vector vec2 = Vector.ByTwoPoints(ptList[k], ptList[j]);
                        vec2 = vec2.Normalized();
                        vec2 = vec2.Scale(diff2);
                        Point p2 = ptList[k].Add(vec2);
                        ptList[k] = p2;
